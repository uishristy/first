<?php session_start();
    ...
    if (file_exists($target_file)) {
        header('Location:add_plant.php?choice=success&value=Sorry File Already Exist');
        exit();
    }
    // Check file size
    if ($_FILES["thumbnail"]["size"] > 500000000) {
        header('Location:add_plant.php?choice=success&value=Sorry File too Large ');
        exit();
    }
    // Allow certain file formats
    if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
    && $imageFileType != "gif"  && $imageFileType != "pdf") {
      header('Location:add_plant.php?choice=success&value=Sorry, only JPG, JPEG, PNG & GIF files are allowed.');       
        exit();
    }
    // Check if $uploadOk is set to 0 by an error
    if ($uploadOk == 0) {
    //echo "mastar";
         header('Location:add_plant.php?choice=success&value=Sorry, your File was Not Uploaded');
         exit();
    } else {
        if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $target_file)) {
          ...
          $main_thumb_file = $dir."/main_thumb/".$file;
           smart_resize_image($file_src , null, 400 , 500 , false , $main_thumb_file , false , false ,80 ); 
          $small_thumb_file = $dir."/small_thumb/".$file;
          smart_resize_image($file_src , null, 400 , 300 , false , $small_thumb_file , false , false ,80 ); 
         ...
         //print_r($arrayt);
           ...
           $table = "plant_products";
              ...
              header('Location:view_plant.php?choice=success&value=product Added Successfully');              
              exit();
        } else {
          header('Location:view_plant.php?choice=success&value=Sorry, There Was an Error Uploading Your File');
           exit();
        }
    }
